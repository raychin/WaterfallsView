...
import r.com.rRefreshLayoutLib.R;
 ...
 * @date 2018/01/24
 */
public class RyRefreshLayout extends FrameLayout {
    ...
    private RyRefreshLayoutListener ryRefreshLayoutListener;
    private RyRefreshLayoutController ryRefreshLayoutController;
    /**
     * 是否支持下拉刷新
     */
    ...
    public RyRefreshLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
        initAttrs(context, attrs);
    }

    public RyRefreshLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        initAttrs(context, attrs);
    }

    @TargetApi(Build.VERSION_CODES.LOLLIPOP)
    public RyRefreshLayout(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        ...
        loadingViewFinalHeight = RyUtils.dipToPx(context, LOADING_VIEW_FINAL_HEIGHT_DP);
        ...
        int resId;
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.RyRefreshLayout);
        Resources resources = context.getResources();

        // LoadView背景颜色
        resId = ta.getResourceId(R.styleable.RyRefreshLayout_load_view_bg_color, -1);
        if (resId == -1) {
            mLoadViewBgColor = ta.getColor(R.styleable.RyRefreshLayout_load_view_bg_color,
                    Color.WHITE);
        } else {
        ...
        // 加载文字颜色
        resId = ta.getResourceId(R.styleable.RyRefreshLayout_load_text_color, -1);
        if (resId == -1) {
            mLoadViewTextColor = ta.getColor(R.styleable.RyRefreshLayout_load_text_color,
                    Color.BLACK);
        } else {
        ...
        // 进度条背景颜色
        resId = ta.getResourceId(R.styleable.RyRefreshLayout_progress_bg_color, -1);
        if (resId == -1) {
            mProgressBgColor = ta.getColor(R.styleable.RyRefreshLayout_progress_bg_color,
                    Color.WHITE);
        } else {
        ...
        // 进度条颜色
        resId = ta.getResourceId(R.styleable.RyRefreshLayout_progress_bar_color, -1);
        if (resId == -1) {
            mProgressColor = ta.getColor(R.styleable.RyRefreshLayout_progress_bar_color,
                    Color.RED);
        } else {
        ...
        // 下拉刷新文字描述
        resId = ta.getResourceId(R.styleable.RyRefreshLayout_pull_refresh_text, -1);
        if (resId == -1) {
            mPullRefreshText = ta.getString(R.styleable.RyRefreshLayout_pull_refresh_text);
        } else {
            mPullRefreshText = resources.getString(resId);
        }

        // 上拉加载文字描述
        resId = ta.getResourceId(R.styleable.RyRefreshLayout_pull_load_text, -1);
        if (resId == -1) {
            mPullLoadText = ta.getString(R.styleable.RyRefreshLayout_pull_load_text);
        } else {
            mPullLoadText = resources.getString(resId);
        }

        mAutoLoadMore = ta.getBoolean(R.styleable.RyRefreshLayout_auto_load_more, false);
        mPullRefreshEnable = ta.getBoolean(R.styleable.RyRefreshLayout_pull_refresh_enable, true);
        mPullLoadEnable = ta.getBoolean(R.styleable.RyRefreshLayout_pull_load_enable, true);
    }
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (ev.getActionMasked()) {
            case MotionEvent.ACTION_DOWN: {
                if (ryRefreshLayoutController != null) {
                    mPullRefreshEnable = ryRefreshLayoutController.isPullRefreshEnable();
                    mPullLoadEnable = ryRefreshLayoutController.isPullLoadEnable();
                }
                preY = ev.getY();
                preX = ev.getX();
    ...
    private void startPullDownRefresh(int headerViewHeight) {
        ...
        animator.addListener(new SimpleAnimatorListener() {
            ...
            public void onAnimationEnd(Animator animation) {
                ...
                if (ryRefreshLayoutListener != null) {
                    ryRefreshLayoutListener.onRefresh();
                }
            }
        });
    ...
    private void startPullUpLoadMore(int headerViewHeight) {
        ...
        animator.addListener(new SimpleAnimatorListener() {
            ...
            public void onAnimationEnd(Animator animation) {
                ...
                if (ryRefreshLayoutListener != null) {
                    ryRefreshLayoutListener.onLoadMore();
                }
            }
        });
        animator.setDuration(300);
    ...
    public void setRefreshLayoutListener(RyRefreshLayoutListener refreshLayoutListener) {
        this.ryRefreshLayoutListener = refreshLayoutListener;
    }

    public interface RyRefreshLayoutListener {
        /**
         * 下拉刷新回调
         */
        void onRefresh();
    ...
    public void setRefreshLayoutController(RyRefreshLayoutController refreshLayoutController) {
        this.ryRefreshLayoutController = refreshLayoutController;
    }

    public interface RyRefreshLayoutController {
        /**
         * 当前下拉刷新是否可用
         * @return 是否支持下拉刷新
         */
